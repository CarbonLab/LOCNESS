...
if BIC_changepoints == 0
    ...
    valid = ~isnan(data.ESPER.referenceAnomalyTimeSeries);
    p     = polyfit(profile(valid), data.ESPER.referenceAnomalyTimeSeries(valid), 1);
    linfit = polyval(p, profile);
    TF    = false(size(profile)); % no change points

else
    [TF,S1,S2] = ischange(data.ESPER.referenceAnomalyTimeSeries,'linear','MaxNumChanges',BIC_changepoints);
    S1_full    = fillmissing(S1, 'nearest');
    S2_full    = fillmissing(S2, 'nearest');
    linfit     = S1_full .* profile + S2_full;
end

data.ph.anomaly_corrected   = data.ESPER.referenceAnomalyTimeSeries - linfit;
data.ph.ph_corrected        = cell(size(data.ph.ph));

for ii = 1:numel(data.ph.ph)
    data.ph.ph_corrected{ii}        = data.ph.ph{ii} - linfit(ii);
    data.ph.ph_correctedAtDepth(ii) = data.ph.phAtDepth(ii) - linfit(ii);
end
...
ax2 = nexttile(tl,2);
plot(profile, data.ESPER.referenceAnomalyTimeSeries, 'ko')
hold on
plot(profile, linfit, 'r-', 'LineWidth', 2)
if any(TF)
    xline(profile(TF))
    legend('Ref Anom', 'Drift Fit', 'Change Points', Location='southeast')
else
    legend('Ref Anom', 'Drift Fit', Location='southeast')
end
title('Reference Anomaly')
ylabel('\DeltapH (meas - ESPER)')
ylim([-.05 .05])
...
ax4 = nexttile(tl,4);
plot(profile, data.ph.anomaly_corrected, 'ko')
hold on
yline(-0.01, 'r--')
yline( 0.01, 'r--')
yline(0, 'k', LineStyle=':')
...
figname = fullfile("figures",sprintf('SN%s_BIC%d_pH_Correction.png',SN,BIC_changepoints));
