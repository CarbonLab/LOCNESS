...
if glider == 2
previousDiveDepth = max(t.ctd.p{end-1}) ;
...
elseif glider == 1
previousDiveDepth = max(t.depth{end-1});
radiusThreshold = 5 * previousDiveDepth./1000; % six times previous dive's depth - can update
...
% predict next lon/lat by just adding dlat or dlon to the last surfacing
    ...
    if(distwp1<radiusThreshold)
        ...
        % assume its going to the next waypoint
            if ~isempty(idx) && (idx+1) <= 5 %size(allWpts,1) seem to only have 4 waypoints at a time
                newWpt = allWpts(idx + 1, :);
                lat_waypoint = newWpt(1) ;
                lon_waypoint = newWpt(2) ;
                disp('Nearing waypoint - new waypoint found.');
